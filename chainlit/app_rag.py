...
class DocumentChain:
    ...
    def init_retriever(self):
        ...
        self.vectorstore = Chroma(collection_name="split_parents",
                                  embedding_function=self.bge_embeddings, persist_directory="doc_db/")
...
class ConversationChain():
    ...
    def start_new_conversation(self):
        ...
        self.current_conversation_file = os.path.join(
            self.directory, conversation_filename)
    ...
    def init_retriever(self):
        ...
        child_splitter = RecursiveCharacterTextSplitter(chunk_size=500)
        memory_parent_splitter = RecursiveCharacterTextSplitter(
            chunk_size=1500, chunk_overlap=250)
        memory_child_splitter = RecursiveCharacterTextSplitter(chunk_size=500)
        fs = LocalFileStore("./conv_store")
        self.store = create_kv_docstore(fs)
        self.vectorstore = Chroma(
            collection_name="old_conv", embedding_function=self.bge_embeddings, persist_directory="conv_db/")
        self.memory_vectorstore = Chroma(
            collection_name="current_conv", embedding_function=self.bge_embeddings)
...
async def on_message(message: cl.Message):
    ...
    if current_conversation == "":
        ...
        try:
            current_memory = conversation_chain.memory.get_relevant_documents(user_input)[
                0].page_content
            print("\n \n")
    ...
    if chain.done and conversation_chain.done:
        ...
        response_conv = conversation_chain.retriever.get_relevant_documents(
            user_input)
